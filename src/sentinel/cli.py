...
def paper_buy(
    ...
    Never places an order anywhere — Sentinel has no broker connection. A risk
    limit the position breaks is WARNED about and recorded anyway; input that
    can only be a mistake (stop above entry, the same fill twice) is refused.
    The same rules drive the dashboard's Record-a-trade form: one
    implementation, `portfolio.manual`.
    """
    ...
    conn = _db(config, create=True)
    try:
        result = manual.record_buy(
            conn, config, ticker=ticker, shares=shares, price=dec(price),
            opened_on=dt.date.fromisoformat(date) if date else None,
            stop=dec(stop) if stop else None, idea_class=IdeaClass(idea_class),
            note=note,
        )
    ...
    console.print(f"recorded {shares} × {position.ticker} @ {position.entry} "
                  f"{position.currency} (£{result.gbp_amount:,.2f}) · cash £{result.cash:,.2f}")
...
def paper_sell(
    ...
    conn = _db(config)
    try:
        result = manual.record_sell(
            conn, config, ticker=ticker, price=dec(price),
            closed_on=dt.date.fromisoformat(date) if date else None,
        )
    except manual.ManualEntryError as exc:
        console.print(f"[red]{exc}[/]")
        raise typer.Exit(EXIT_FAILURE) from exc
    position = result.position
    console.print(f"closed {position.shares} × {position.ticker} @ {position.exit_price} · "
                  f"P&L £{result.pnl:,.2f} · cash £{result.cash:,.2f}")
    conn.close()
